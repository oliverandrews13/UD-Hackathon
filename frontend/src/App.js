            ...
            {buildings.map(b => (
              <div key={b.id} style={{
                ...
                <div style={{ fontSize: 13, color: "#94a3b8" }}>
                  {b.type === "source" ? `Generates: ${b.supply} kWh` : `Demand: ${b.demand} kWh`}
                </div>
                {b.flagged && (
      ...
      {activeTab === "optimizer" && (
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
            ...
            <span style={{ color: "#94a3b8", fontSize: 13 }}>Allocates solar energy first, grid as fallback</span>
          </div>
                  ...
                  return (
                    <div key={a.building_id} style={{
                      ...
                      <div style={{ background: "#334155", borderRadius: 4, height: 8, overflow: "hidden" }}>
                        <div style={{ width: `${solarPct}%`, height: "100%", background: "#facc15", borderRadius: 4 }} />
                      </div>
      ...
      {activeTab === "issues" && (
        <div>
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <select
                ...
                style={{ width: "100%", padding: "8px 12px", borderRadius: 8, background: "#0f172a", border: "1px solid #334155", color: "#e2e8f0", fontSize: 14 }}
              >
                <option value="">Select building...</option>
                {consumers.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
            <div style={{ marginBottom: 12 }}>
            ...
            <button
              onClick={submitIssue}
              style={{ padding: "10px 24px", background: "#ef4444", color: "#fff", border: "none", borderRadius: 8, fontWeight: 600, cursor: "pointer", fontSize: 14 }}
            >
              🚨 Flag Issue
            </button>
          </div>
          ...
          {issues.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {issues.map((issue, i) => (
                <div key={i} style={{ background: "#1e293b", borderRadius: 10, padding: "12px 16px", border: "1px solid #ef4444" }}>
